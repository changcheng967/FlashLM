#!/usr/bin/env python3
"""
FlashLM v8.2 — CORTEX + Value Heads, Subset Training
=====================================================
Same architecture as v8.1 (CORTEX-VIII + lookahead value heads).
Key change: train on a 20M token subset = ~1 epoch in 2 hours
  (vs 0.027 epochs on the full 574M token dataset)

Why: The TinyStories paper proved 1.7M params can generate coherent
stories with 1+ full epoch of training. Our v8.1 model reached PPL 2.40
but generated incoherent text because it only saw 2.7% of the data.
It learned token statistics but not story patterns.

Also adds improved generation: nucleus sampling + frequency penalty.

Usage:  python v8/train_v82.py                # 2 hours
        python v8/train_v82.py --minutes 7    # quick test
"""
...
# ============================================================================
VOCAB_SIZE = 4096
SUBSET_TOKENS = 20_000_000   # train on first 20M tokens (~73K stories)

# CORTEX-VIII architecture
D_MODEL = 256
N_LAYERS = 6
D_FF = 640                    # wider FFN — stronger expressiveness (net compute ~same)
N_HEADS = 4
D_HEAD = 64
SWA_WINDOW = 32               # smaller window — TinyStories has short-range deps
D_MEM = 32
SEQ_LEN = 256
BATCH_SIZE = 4
GRAD_ACCUM = 8
...
def train(tokenizer, vocab, train_ds, val_data, minutes):
    ...
    json.dump({'model': 'FlashLM v8.2 SearchLM subset',
...
def main():
    parser = argparse.ArgumentParser(description="FlashLM v8.2 Subset Training")
    parser.add_argument('--minutes', type=float, default=120)
    args = parser.parse_args()

    print(f"\n{'=' * 70}")
    print(f"  FlashLM v8.2 — CORTEX + Value Heads, Subset Training")
    print(f"{'=' * 70}")
    print(f"  Subset: {SUBSET_TOKENS/1e6:.0f}M tokens (~1 epoch in 2h)")
