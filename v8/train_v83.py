#!/usr/bin/env python3
"""
FlashLM v8.3 — Coherent Generation Focus
==========================================
v8.2 broke the repetition loops (no more "Lily x20") but grammar is still broken.
v8.3 changes:
  - 10M token subset = ~1.6 epochs in 2h (vs 0.6 in v8.2)
  - Revert D_FF to 512 (restore 2000+ tok/s throughput)
  - Keep SWA_WINDOW=32, entropy reg, zero embed WD (proven effective)
  - Better token decoding in generation output

Usage:  python v8/train_v83.py                # 2 hours
        python v8/train_v83.py --minutes 7    # quick test
"""
...
D_MODEL = 256
N_LAYERS = 6
D_FF = 512                    # revert from 640 — restore 2000+ tok/s throughput
N_HEADS = 4
D_HEAD = 64
...
def train(tokenizer, vocab, train_ds, val_data, minutes):
    ...
    json.dump({'model': 'FlashLM v8.3 Coherent Generation Focus',
...
def main():
    parser = argparse.ArgumentParser(description="FlashLM v8.3 Coherent Generation Focus")
    parser.add_argument('--minutes', type=float, default=120)
    args = parser.parse_args()

    print(f"\n{'=' * 70}")
    print(f"  FlashLM v8.3 — Coherent Generation Focus")
    print(f"{'=' * 70}")
    print(f"  Subset: {SUBSET_TOKENS/1e6:.0f}M tokens (~1.6 epochs in 2h)")
