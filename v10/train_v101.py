#!/usr/bin/env python3
"""
FlashLM Vortex v10.1 — Maximum Throughput Training
====================================================

Goal: Achieve coherent English text in 2h on 4 vCPU.
Strategy: maximize tokens/second by minimizing per-step compute.

Architecture: Standard causal attention + SwiGLU + RMSNorm + weight tying
- 2 layers (not 4) — 1.47x faster, profiling-verified
- d_ff=512 (not 768) — FFN is 70% of compute, 33% reduction
- No dropout — 0 quality cost at this scale, 8% speedup
- No BitLinear — 2.5% overhead with no speed benefit
- torch.compile — 21% speedup verified on this server
- Progressive layer freezing — freeze layer 0 after 30min

Target: 8,100+ tok/s → 58M+ tokens in 2h (v5.2 coherence threshold: ~50M)

Data: Full TinyStories V2-GPT4 train split (~550M tokens).

Usage:
  python v10/train_v101.py --minutes 120
"""
...
class VortexModel(nn.Module):
    def __init__(self, vocab, d_model, d_ff, n_heads, d_head, n_layers, seq_len):
        ...
        print(f"  Model: Vortex v10.1 | {total:,} ({total/1e6:.2f}M)")
...
def train(tokenizer, vocab, train_ds, val_data, minutes):
    ...
    print(f"  Batch: {BATCH_SIZE * GRAD_ACCUM} | Freeze layer 0 after {FREEZE_AFTER_MINUTES}min")
    print(f"  No dropout | No BitLinear | torch.compile enabled\n")
    ...
    while True:
        ...
        if not layer0_frozen and (time.time() - t0) > FREEZE_AFTER_MINUTES * 60:
            print(f"\n  *** Freezing layer 0 at step {step} ({(time.time()-t0)/60:.1f}m) ***")
...
def main():
    parser = argparse.ArgumentParser(description="FlashLM Vortex v10.1")
    ...
    print("=" * 60)
    print("FlashLM Vortex v10.1 — Maximum Throughput")
    print(f"  Standard Linear | d={D_MODEL} L={N_LAYERS} H={N_HEADS} d_head={D_HEAD} d_ff={D_FF}")
    print(f"  No dropout | torch.compile | Progressive freezing")
