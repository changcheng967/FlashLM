#!/usr/bin/env python3
"""
CPUFlow v6 — Learned exponential decay cumsum.
Single change over v5-LN: cumsum → exponential decay (recency bias + gradient balance).
The decay factor is learned per-dim per-layer, ∈ (0.9, 1.0).
"""
...
class DecayScanBlock(nn.Module):
    def __init__(self, d, k, d_ff):
        super().__init__()
        self.norm = nn.LayerNorm(d)
        self.W_proj = nn.Linear(d, 3 * k, bias=False)
        self.log_decay = nn.Parameter(torch.zeros(k))
    ...
    def forward(self, x):
        ...
        # Learned decay: sigmoid ensures (0,1), scale to (0.9, 1.0)
        decay = 1.0 - 0.1 * torch.sigmoid(self.log_decay)  # [K]

        # Vectorized exponential decay cumsum:
        # h[t] = decay * h[t-1] + x[t]
        # = sum_{s=0}^{t} decay^(t-s) * x[s]
        # = decay^t * cumsum(x[s] * decay^(-s))
        t_range = torch.arange(T, device=x.device, dtype=x.dtype)
        # Use log-space for numerical stability
        log_decay = torch.log(decay.clamp(min=1e-7))
        powers = torch.exp(t_range.unsqueeze(1) * log_decay.unsqueeze(0))  # [T, K]
        inv_powers = torch.exp((-t_range.unsqueeze(1)) * log_decay.unsqueeze(0))  # [T, K]

        gated = key * value
        num = torch.cumsum(gated * inv_powers.unsqueeze(0), dim=1) * powers.unsqueeze(0)
        den = torch.cumsum(key * inv_powers.unsqueeze(0), dim=1) * powers.unsqueeze(0) + SCAN_EPS
        s = query * num / den
...
class CPUFlowV6(nn.Module):
    def __init__(self, vocab, d, k, n_layers):
        ...
        self.blocks = nn.ModuleList([DecayScanBlock(d, k, D_FF) for _ in range(n_layers)])
...
def train(args):
    ...
    print(f"\n{'='*70}")
    print(f"CPUFlow v6 — Learned Decay Cumsum")
    print(f"{'='*70}")
    print(f"  d={D}, k={K}, d_ff={D_FF}, {N_LAYERS} layers")
    print(f"  Learned exponential decay cumsum (per-dim, per-layer)")
    ...
    model = CPUFlowV6(vocab, D, K, N_LAYERS).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"  Parameters: {n_params:,}")

    for i, block in enumerate(model.blocks):
        decay = 1.0 - 0.1 * torch.sigmoid(block.log_decay)
        print(f"  Layer {i} decay: min={decay.min():.4f} max={decay.max():.4f} mean={decay.mean():.4f}")
    ...
    total_tokens = 0
    nan_count = 0
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cpuflow_v6_decay_best.pt')
    ...
    while True:
        ...
        if step % EVAL_EVERY == 0:
            ...
            for i, block in enumerate(model.blocks):
                decay = 1.0 - 0.1 * torch.sigmoid(block.log_decay)
                print(f"      L{i} decay: min={decay.min():.4f} max={decay.max():.4f} mean={decay.mean():.4f}")
    ...
    for i, block in enumerate(model.blocks):
        decay = 1.0 - 0.1 * torch.sigmoid(block.log_decay)
        print(f"  Layer {i}: {decay.detach().cpu().numpy().round(4)}")
