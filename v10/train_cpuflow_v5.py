#!/usr/bin/env python3
"""
CPUFlow v5 — Gated Normalized CumSum with PowerNorm
====================================================
v4 failed (PPL 311) due to three critical issues:
  1. Bidirectional cumsum leaks future tokens in causal LM
  2. No normalization — cumsum grows unboundedly through layers
  3. Multi-stream decays all converge to same value — no differentiation

v5 fixes:
  - Forward-only (causal) cumsum
  - Linear attention normalization: cumsum(gate*value) / cumsum(gate)
  - PowerNorm before each block (lightweight, 2 ops)
  - Single-stream with query for content-dependent readout

Architecture per ScanBlock:
  x_n = PowerNorm(x)            # lightweight normalization
  h = W_proj(x_n)             # fused: d -> 3*k (query + key + value)
  ...
  den = cumsum(key) + eps     # running normalizer
...
class PowerNorm(nn.Module):
    """Learned power normalization: x / Lp(x) * w."""
...
class ScanBlock(nn.Module):
    """
    Gated normalized cumsum with self-mix + FFN.
    PowerNorm -> fused Q/K/V -> gated cumsum -> self-mix -> expand -> FFN.
    """
    def __init__(self, d, k, d_ff):
        super().__init__()
        self.norm = PowerNorm(d)
        # Fused projection: d -> 3*k
        self.W_proj = nn.Linear(d, 3 * k, bias=False)
        # Self-mix in compressed space
        self.W_m = nn.Linear(k, k, bias=False)
        # Output projection: k -> d
...
def train(args):
    ...
    print(f"\n{'='*70}")
    print(f"CPUFlow v5 — Gated Normalized CumSum + PowerNorm")
    print(f"{'='*70}")
    print(f"  d={D}, k={K}, d_ff={D_FF}, {N_LAYERS} layers")
    print(f"  Fused projection: d->{3*K} (query+key+value)")
    print(f"  PowerNorm per block, forward-only (causal), ReLU FFN")
        ...
        if step % EVAL_EVERY == 0:
            ...
            if val_ppl < best_ppl:
                ...
                torch.save({
                    ...
                    'val_ppl': val_ppl,
                    'n_params': n_params,
                }, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cpuflow_v5_best.pt'))
