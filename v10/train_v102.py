#!/usr/bin/env python3
"""
FlashLM Vortex v10.2 — Fixed Architecture
===========================================

Two critical bugs fixed from v10/v10.1:
1. LR schedule: now uses estimated total steps, not seconds (LR no longer bottoms at step 7200)
2. Positional encoding: RoPE added (model can now learn word order)

Architecture: 3L, d=256, d_head=64, d_ff=512, RoPE, torch.compile
Target: 30-min validation run → if PPL drops to 3-6, commit to full 2h

Usage:
  python v10/train_v102.py --minutes 30
  python v10/train_v102.py --minutes 120
"""
...
class VortexModel(nn.Module):
    def __init__(self, vocab, d_model, d_ff, n_heads, d_head, n_layers, seq_len):
        ...
        print(f"  Model: Vortex v10.2 | {total:,} ({total/1e6:.2f}M)")
        print(f"    d={d_model}, L={n_layers}, H={n_heads}, d_head={d_head}, d_ff={d_ff}")
        print(f"    RoPE | Linear decay LR | N-gram blocking")
...
def get_lr(step, warmup, max_lr, min_lr, total_steps):
    # Linear decay (BabyLM 2025: linear beats cosine at small scale)
    if step < warmup:
...
def train(tokenizer, vocab, train_ds, val_data, minutes):
    ...
    # BUG FIX: estimate total steps from expected throughput
    estimated_tps = 5500  # conservative for 3L/512/compile on 4 vCPU
...
def main():
    parser = argparse.ArgumentParser(description="FlashLM Vortex v10.2")
    ...
    print("=" * 60)
    print("FlashLM Vortex v10.2 — Fixed Architecture")
