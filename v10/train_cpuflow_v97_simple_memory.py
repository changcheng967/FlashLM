#!/usr/bin/env python3
"""
CPUFlow v9.7 — Simple Memory Enhancement (No Entity Routing)

v9.6 showed: two-phase training with entity routing gives PPL 10.88 but addresses collapse.
The memory sidepath itself adds value (10.88 vs v5-LN's 11.94) without entity specificity.

This version: just add RAM-Net memory to v5-LN backbone, train normally.
No contrastive loss, no entity routing, no phase training.
Single-phase training with CE + FSP, same as v5-LN but with memory sidepath.

Goal: test if memory-enhanced v5-LN improves coherence (not just PPL).
"""
...
def train(args):
    ...
    print(f"\n{'=' * 70}")
    print(f"CPUFlow v9.7 — Simple Memory Enhancement (No Entity Routing)")
    print(f"{'=' * 70}")
    print(f"  d={D}, k={K}, d_ff={D_FF}, {N_LAYERS} layers")
    print(f"  Memory: M={M_SLOTS} slots, U={N_SUB}, d_p={D_SUB}, K={K_TOP}, d_v={D_VAL}")
    print(f"  Training: single-phase CE + FSP, no routing loss")
    ...
    model = CPUFlowV97(vocab, D, K, N_LAYERS).to(device)
    ...
    best_ppl = float('inf')
    total_tokens = 0
    ckpt_path = os.path.join(ckpt_dir, 'cpuflow_v97_best.pt')
    ...
    while True:
        ...
        if step % EVAL_EVERY == 0:
            ...
            elapsed = (time.time() - t0) / 60
            toks = total_tokens / (time.time() - t0)
            print(f"  >>> step {step:5d} | VAL PPL {val_ppl:.2f} | {toks:.0f} tok/s | {elapsed:.1f}m")
            if val_ppl < best_ppl:
                best_ppl = val_ppl
                torch.save({
