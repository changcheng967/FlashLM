#!/usr/bin/env python3
"""
CPUFlow v7 Warm-Start — Fine-tune v5-LN with memory bank.

v7 from-scratch failed: gate collapse (gates stayed at 0.12, memory unused).
Fix: warm-start from v5-LN checkpoint (PPL 11.94), open gates (bias=+1.0),
no diversity loss for first 30 min so memory bank can learn freely.
"""
...
D_K_MEM = 32
D_V_MEM = 64
GATE_BIAS = 1.0       # OPEN gates (sigmoid=0.73) — reversed from cold start
DIVERSITY_W = 0.01
DIV_WARMUP_MIN = 30    # no diversity loss for first N minutes
...
class MemoryScanBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # --- Memory bank ---
        mem_h = self.W_mem(x_n)
        q_mem, gate_raw, proposal = mem_h.split(
            [D_K_MEM, self.m_slots, self.d_v_mem], dim=-1)

        scores = F.linear(q_mem, self.memory_K)
        alpha = F.softmax(scores, dim=-1)

        gate = torch.sigmoid(gate_raw + self.gate_bias)
        write_w = alpha * gate

        r_base = alpha @ self.memory_V
        cross = alpha @ write_w.transpose(1, 2)
        causal_cross = torch.tril(cross, diagonal=-1)
        r_write = torch.bmm(causal_cross, proposal)
        r = r_base + r_write

        s = self.W_merge(torch.cat([s, r], dim=-1))
        s = self.W_m(s)
        x = x + self.W_out(s)
...
class CPUFlowV7(nn.Module):
    ...
    def slot_diagnostics(self):
        ...
        for block in self.blocks:
            ...
            gates.append(torch.sigmoid(block.gate_bias).mean().item())
        return sum(sims) / len(sims), sum(gates) / len(gates)


def load_v5ln_weights(model, ckpt_path):
    """Load matching weights from v5-LN checkpoint into v7 model."""
    ...
    # Override gate bias to open
    for block in model.blocks:
        block.gate_bias.data.fill_(GATE_BIAS)

    print(f"  Loaded {loaded} params from v5-LN (skipped {skipped} new memory params)")
    print(f"  v5-LN checkpoint PPL: {ckpt.get('val_ppl', '?')}, step: {ckpt.get('step', '?')}")
    return ckpt.get('val_ppl', '?')
...
def train(args):
    ...
    print(f"\n{'='*70}")
    print(f"CPUFlow v7 Warm-Start — v5-LN + Memory Bank")
    print(f"{'='*70}")
    print(f"  d={D}, k={K}, d_ff={D_FF}, {N_LAYERS} layers")
    print(f"  Memory bank: {M_SLOTS} slots, d_k={D_K_MEM}, d_v={D_V_MEM}")
    print(f"  Gate bias: {GATE_BIAS} (sigmoid={torch.sigmoid(torch.tensor(GATE_BIAS)):.3f}, OPEN)")
    print(f"  Diversity warmup: {DIV_WARMUP_MIN} min (no diversity loss)")
    ...
    # Warm-start from v5-LN
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cpuflow_v5_ln_best.pt')
    if os.path.exists(ckpt_path):
        v5_ppl = load_v5ln_weights(model, ckpt_path)
    else:
        print(f"  WARNING: No v5-LN checkpoint at {ckpt_path}, training from scratch")

    slot_sim, gate_eff = model.slot_diagnostics()
    print(f"  Slot similarity: {slot_sim:.4f}, Gate effective: {gate_eff:.3f}")

    for i, block in enumerate(model.blocks):
        print(f"  L{i} gate: bias={block.gate_bias.mean():.2f} eff={torch.sigmoid(block.gate_bias).mean():.3f}")

    # Lower LR for fine-tuning (scan already trained)
    ft_lr = LR * 0.5
    ...
    total_tokens = 0
    nan_count = 0
    save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cpuflow_v7_warm_best.pt')

    print(f"\n  Fine-tuning for {args.minutes} minutes (lr={ft_lr})")
    print(f"  BATCH={BATCH}x{GRAD_ACC}, CLIP={CLIP}")
    print("-" * 70)
    ...
    while True:
        ...
        div_w = DIVERSITY_W if elapsed_min >= DIV_WARMUP_MIN else 0.0
        ...
        for ga_i in range(GRAD_ACC):
            ...
            loss = ce + FSP_ALPHA * fsp + div_w * div
        ...
        if step % LOG_EVERY == 0:
            ...
            slot_sim, gate_eff = model.slot_diagnostics()
            print(f"  step {step:5d} | CE {ce_avg:.4f} PPL {ppl:8.2f} | FSP {fsp_avg:.4f} | div {div_avg:.6f} | slot_sim {slot_sim:.4f} gate {gate_eff:.3f} | grad {total_grad_norm:.4f} | {toks_per_s:.0f} tok/s | {elapsed:.1f}m")
        if step % EVAL_EVERY == 0:
            val_ppl = evaluate(model, val_data, device)
            elapsed = (time.time() - t0) / 60
            toks_per_s = total_tokens / (time.time() - t0)
            slot_sim, gate_eff = model.slot_diagnostics()
            print(f"  >>> step {step:5d} | VAL PPL {val_ppl:.2f} | slot_sim {slot_sim:.4f} gate {gate_eff:.3f} | {toks_per_s:.0f} tok/s | {elapsed:.1f}m")
            if val_ppl < best_ppl:
                best_ppl = val_ppl
                torch.save({
            ...
            for i, block in enumerate(model.blocks):
                ...
                g_eff = torch.sigmoid(block.gate_bias).mean().item()
                print(f"      L{i}: slot_sim={avg_sim:.4f} gate_eff={g_eff:.3f}")
    ...
    for i, block in enumerate(model.blocks):
        ...
        print(f"  L{i}: slot_sim={avg_sim:.4f} gate_eff={g_eff:.3f} key_var={key_var:.6f}")
