#!/usr/bin/env python3
"""
CPUFlow v8.5 — Entity Routing Supervision

v8 achieved PPL 9.30 (best CPU-native) but slot entropy stayed 3.43/3.47 (uniform).
The routing never learned entity clustering because no training signal rewards it.

v8.5 adds a routing consistency loss: when the same entity token (e.g. "Lily")
appears at multiple positions, their routing distributions are pushed toward agreement.
This gives the STE-based routing an explicit training signal for entity specialization.

Vectorized implementation: same-token pairs identified via [B,T,T] comparison,
routing similarity measured as dot product of softmax distributions.
"""
...
ENTITY_ROUTE_W = 0.05  # entity routing consistency loss weight
...
class DiscreteScanBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        self.last_route_logits = q_route  # stored for entity routing loss
...
def precompute_entity_tokens(tokenizer, vocab_size):
    """Identify token IDs that look like entity tokens (start with uppercase, not common words)."""
    common = {'The', 'A', 'An', 'In', 'On', 'At', 'It', 'He', 'She', 'We', 'They',
              'My', 'His', 'Her', 'Our', 'Their', 'This', 'That', 'These', 'Those',
              'And', 'But', 'Or', 'So', 'If', 'When', 'Then', 'There', 'Here',
              'What', 'Who', 'How', 'Why', 'Where', 'Which', 'Not', 'No', 'Yes',
              'One', 'Two', 'All', 'Some', 'Each', 'Every', 'Many', 'Much'}
    entity_ids = set()
    for i in range(vocab_size):
        tok = tokenizer.id_to_token(i)
        if tok is None:
            continue
        # Decode: handle byte-fallback tokens (Ġ = space prefix in BPE)
        text = tok.replace('Ġ', ' ').replace('Ċ', '\n').strip()
        if len(text) >= 2 and text[0].isupper() and text not in common:
            entity_ids.add(i)
    return entity_ids


def compute_entity_routing_loss(model, input_ids, entity_ids):
    """Push same-entity tokens toward same slot routing.

    For each pair of positions (i,j) where input_ids[b,i] == input_ids[b,j]
    and that token is an entity, maximize dot product of their routing distributions.
    """
    if not entity_ids:
    ...
    # Collect routing probabilities from all blocks (average)
    all_soft = []
    for block in model.blocks:
        q_soft = F.softmax(block.last_route_logits / TAU, dim=-1)  # [B, T, M]
        all_soft.append(q_soft)
    avg_soft = torch.stack(all_soft, dim=0).mean(dim=0)  # [B, T, M]

    # Same-token mask: [B, T, T]
    # same_tok[b,i,j] = 1 iff ids[b,i] == ids[b,j]
    ids_expanded_i = input_ids.unsqueeze(2)  # [B, T, 1]
    ids_expanded_j = input_ids.unsqueeze(1)  # [B, 1, T]
    same_tok = (ids_expanded_i == ids_expanded_j).float()  # [B, T, T]

    # Entity mask: [B, T] -> which positions are entity tokens
    eid_tensor = torch.tensor(list(entity_ids), device=dev, dtype=torch.long)
    is_entity = torch.zeros(B, T, device=dev)
    for eid in eid_tensor:
        is_entity += (input_ids == eid).float()
    is_entity = (is_entity > 0).float()

    # Valid pairs: both positions are entities AND same token AND i != j
    entity_mask_2d = is_entity.unsqueeze(2) * is_entity.unsqueeze(1)  # [B, T, T]
    eye = torch.eye(T, device=dev).unsqueeze(0)  # [1, T, T]
    valid_pairs = same_tok * entity_mask_2d * (1 - eye)  # [B, T, T]

    n_pairs = valid_pairs.sum()
    if n_pairs < 1:
        return torch.tensor(0.0, device=dev)

    # Routing similarity: dot product of softmax distributions
    sim = torch.bmm(avg_soft, avg_soft.transpose(1, 2))  # [B, T, T]

    # Loss: push same-entity pairs toward agreement (sim → 1)
    loss = ((1 - sim) * valid_pairs).sum() / n_pairs
    return loss
...
def train(args):
    ...
    print(f"\n{'='*70}")
    print(f"CPUFlow v8.5 — Entity Routing Supervision")
    print(f"{'='*70}")
    print(f"  d={D}, k={K}, d_ff={D_FF}, {N_LAYERS} layers")
    print(f"  {M_SLOTS} slots, d_state={D_STATE}, tau={TAU}")
    print(f"  Hard argmax routing (STE gradient), entity consistency loss")
    ...
    entity_ids = precompute_entity_tokens(tokenizer, vocab)
    print(f"  Entity tokens: {len(entity_ids)} IDs identified")
    ...
    total_tokens = 0
    nan_count = 0
    save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cpuflow_v85_entity_route_best.pt')

    print(f"\n  Fine-tuning for {args.minutes} minutes (lr={ft_lr})")
    print(f"  BATCH={BATCH}x{GRAD_ACC}, CLIP={CLIP}, BALANCE_W={BALANCE_W}, ENTITY_ROUTE_W={ENTITY_ROUTE_W}")
    print("-" * 70)
    ...
    while True:
        ...
        for ga_i in range(GRAD_ACC):
            ...
            ent_loss = compute_entity_routing_loss(model, xb, entity_ids)
            loss = ce + FSP_ALPHA * fsp + BALANCE_W * bal + ENTITY_ROUTE_W * ent_loss
        ...
        ent_avg = a_ent / GRAD_ACC
        if step % LOG_EVERY == 0:
            elapsed = (time.time() - t0) / 60
            toks_per_s = total_tokens / (time.time() - t0)
            ppl = math.exp(min(ce_avg, 10))
            slot_ent, max_ent = model.slot_diagnostics()
            print(f"  step {step:5d} | CE {ce_avg:.4f} PPL {ppl:8.2f} | FSP {fsp_avg:.4f} | ENT {ent_avg:.4f} | slot_ent {slot_ent:.2f}/{max_ent:.2f} | grad {total_grad_norm:.4f} | {toks_per_s:.0f} tok/s | {elapsed:.1f}m")
    ...
    print(f"v8 discrete baseline: 9.30 | v5-LN baseline: 11.94")
    print(f"Parameters: {n_params:,}")
