#!/usr/bin/env python3
"""
CPUFlow v9 — RAM-Net Sparse Memory on v5-LN cumsum backbone.

Hypothesis: v5-LN's cumsum provides good language modeling (PPL 11.94) but
lacks entity tracking. RAM-Net's Product Softmax + Top-K addressing adds
explicit entity storage that bypasses the need for learned binding IDs
(which require ~160M params to emerge in standard transformers).

Architecture per layer:
  1. LayerNorm -> fused Q/K/V -> cumsum (language backbone, proven)
  2. Product Softmax -> sparse read from M=512 slot memory (entity retrieval)
  3. Merge via learned gate (model decides cumsum vs memory weight)
  4. Self-mix -> ReLU FFN -> FSP

Key differences from v8 (STE slot routing):
  - No STE: Product Softmax is fully differentiable
  - 512 slots via Kronecker product of 3 sub-softmaxes (vs 32 fixed slots)
  - Top-8 sparse selection (vs top-1 argmax)
  - No mode collapse: independent sub-softmaxes are natural regularizers
"""
...
# Sparse memory (RAM-Net)
N_SUB = 3
D_SUB = 8
M_SLOTS = D_SUB ** N_SUB  # 512
D_VAL = 64
K_TOP = 8
MEM_DECAY = 0.99
...
class RAMScanBlock(nn.Module):
    """v5-LN cumsum backbone + RAM-Net sparse memory sidepath."""
    ...
    def __init__(self, d, k, d_ff, m_slots, n_sub, d_sub, d_val, k_top):
        ...
        # Merge gate
        self.W_merge_gate = nn.Linear(d, k, bias=False)
        self.W_mem_proj = nn.Linear(d_val, k, bias=False)

        # Output + FFN (from v5-LN)
    ...
    def _sparse_memory(self, x_n, top_w, top_idx):
        """Chunked read-write through sparse memory.

        Processes CHUNK positions per iteration instead of 1.
        Within each chunk, all positions read from the same memory state.
        Memory is updated after each chunk (cross-chunk persistence).
        The cumsum backbone handles within-chunk sequential context.
        """
        B, T, _ = x_n.shape
        C = 32
        ...
        for c_start in range(0, T, C):
            ...
            # Read: gather from current memory state (batched over chunk)
            ...
            # Write: aggregate chunk writes and scatter to memory
...
def memory_diagnostics(model):
    ...
    for li, block in enumerate(model.blocks):
        x_n = block.norm.weight  # proxy for checking module exists
        x_n = torch.randn(2, 16, D)
        top_w, top_idx = block._product_softmax_topk(x_n)

        # Slot coverage: unique slots across all positions
        unique_slots = top_idx.reshape(-1).unique().numel()
        stats[f'L{li}_unique_slots'] = unique_slots

        # Address entropy (bits, max=log2(M)=9)
        ...
        stats[f'L{li}_addr_entropy'] = entropy

        # Merge gate mean
        gate = torch.sigmoid(block.W_merge_gate(x_n))
        stats[f'L{li}_merge_gate'] = gate.mean().item()
...
def train(args):
    ...
    print(f"\n{'=' * 70}")
    print(f"CPUFlow v9 — Cumsum + RAM-Net Sparse Memory")
    print(f"{'=' * 70}")
    print(f"  d={D}, k={K}, d_ff={D_FF}, {N_LAYERS} layers")
    print(f"  Memory: M={M_SLOTS} slots, U={N_SUB}, d_p={D_SUB}, K={K_TOP}, d_v={D_VAL}")
    print(f"  Product Softmax: {N_SUB} sub-softmaxes x {D_SUB} = {M_SLOTS} virtual slots")
    print(f"  Decay={MEM_DECAY}")
    ...
    model = CPUFlowV9(vocab, D, K, N_LAYERS).to(device)
    ...
    if args.warm_start and os.path.exists(warm_path):
        ...
        print(f"  Warm-start from v5-LN: loaded {loaded} params, skipped {skipped} (new memory params)")
    ...
    total_tokens = 0
    nan_count = 0
    ckpt_path = os.path.join(ckpt_dir, 'cpuflow_v9_ram_best.pt')
    ...
    while True:
        ...
        if step % LOG_EVERY == 0:
            ...
            avg_unique = np.mean([diag[f'L{i}_unique_slots'] for i in range(N_LAYERS)])
            avg_gate = np.mean([diag[f'L{i}_merge_gate'] for i in range(N_LAYERS)])
            print(f"  step {step:5d} | CE {ce_avg:.4f} PPL {ppl:8.2f} | FSP {fsp_avg:.4f} | "
                  f"grad {total_grad_norm:.4f} | slots {avg_unique:.0f}/{M_SLOTS} | "
                  f"gate {avg_gate:.3f} | {toks_per_s:.0f} tok/s | {elapsed:.1f}m")
    ...
    for li in range(N_LAYERS):
        print(f"  Layer {li}: slots {diag[f'L{li}_unique_slots']}/{M_SLOTS}, "
              f"entropy {diag[f'L{li}_addr_entropy']:.2f} bits, "
              f"merge gate {diag[f'L{li}_merge_gate']:.3f}")
...
if __name__ == '__main__':
    ...
    p.add_argument('--warm_start', action='store_true',
                   help='Warm-start from v5-LN checkpoint')
    args = p.parse_args()
    train(args)
