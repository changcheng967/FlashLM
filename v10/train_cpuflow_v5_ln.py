#!/usr/bin/env python3
"""
CPUFlow v5-LN — Same as v5 but with LayerNorm instead of PowerNorm.
Testing if LayerNorm (27x faster) gives similar PPL.
"""
...
def train(args):
    ...
    print(f"\n{'='*70}")
    print(f"CPUFlow v5-LN — LayerNorm (MKL fused kernel)")
    print(f"{'='*70}")
    print(f"  d={D}, k={K}, d_ff={D_FF}, {N_LAYERS} layers")
    print(f"  LayerNorm 27x faster than PowerNorm")
    ...
    while True:
        ...
        # NaN recovery: reload best checkpoint and reset optimizer
        if step_nan and os.path.exists(ckpt_path):
            nan_count += 1
            print(f"\n  !!! NaN #{nan_count} at step {step} — reloading best checkpoint (PPL {best_ppl:.2f})")
        ...
        if step % EVAL_EVERY == 0:
            ...
            if val_ppl < best_ppl:
                ...
                torch.save({
                    ...
                    'val_ppl': val_ppl,
                    'n_params': n_params,
                }, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cpuflow_v5_ln_best.pt'))
