#!/usr/bin/env python3
"""
FlashLM Vortex v10.3 — Scaled Standard Attention
==================================================

v10.2 proved the architecture works (PPL 25.08, 2.6x from bug fixes) but 3.5M params
is below the coherence threshold. v10.3 scales to ~6.2M params with 6 layers.

Hypothesis: We've never tested standard attention at 7M+ params. v5.2 (5M, PPL 10.56)
and v7.4 (7M CORTEX, PPL 2.33) suggest there's a big gain from scale with standard attn.

Changes from v10.2:
  - 6 layers (was 3)
  - d_ff=768 (was 512)
  - Everything else identical: RoPE, linear decay LR, torch.compile

Usage:
  python v10/train_v103.py --minutes 120
"""
...
OUT_DIR = SCRIPT_DIR / 'out_v103'

# ============================================================================
# CONFIG — scaled from v10.2
# ============================================================================
VOCAB_SIZE = 4096
D_MODEL = 256
D_FF = 768          # was 512
N_HEADS = 4
D_HEAD = 64
SEQ_LEN = 128       # keeping same — clean A/B test
N_LAYERS = 6        # was 3
...
# ============================================================================
# MODEL (identical architecture to v10.2, different hyperparams)
# ============================================================================
...
class VortexModel(nn.Module):
    def __init__(self, vocab, d_model, d_ff, n_heads, d_head, n_layers, seq_len):
        ...
        print(f"  Model: Vortex v10.3 | {total:,} ({total/1e6:.2f}M)")
    ...
    def generate(self, idx, max_new_tokens, temperature=0.7, top_p=0.9,
        ...
        for _ in range(max_new_tokens):
            ...
            # Safety: if all logits are -inf, fall back to greedy
...
def train(tokenizer, vocab, train_ds, val_data, minutes):
    ...
    # Conservative estimate: 6L/d_ff=768 is ~2.5x heavier than 3L/d_ff=512
    estimated_tps = 1800
    ...
    while True:
        ...
        if step % LOG_EVERY == 0:
            ...
            print(f"  step {step:>5d} | CE {avg_ce:.4f} PPL {math.exp(min(avg_ce,10)):.2f} | "
                  f"LR {lr:.1e} | tok/s {tokens_seen/elapsed:.0f} | {elapsed/60:.1f}m")
            running_loss = running_n = 0
...
def main():
    parser = argparse.ArgumentParser(description="FlashLM Vortex v10.3")
    ...
    print("=" * 60)
    print("FlashLM Vortex v10.3 — Scaled Standard Attention")
    print(f"  6L | d={D_MODEL} | d_head={D_HEAD} | d_ff={D_FF} | RoPE")
