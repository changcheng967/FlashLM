#!/usr/bin/env python3
"""
FlashLM Vortex v12 — Narrative Bottleneck Tokens (NBT)
======================================================

v11 SPC result: InfoNCE at stride=20 improved PPL marginally (25.08 → 24.72)
but didn't improve coherence. The 128-dim projection learned surface features,
not narrative structure. Batch negatives were too easy.

v12 NBT: Three key changes from SPC:
  1. 64-dim bottleneck (was 128) — forces harder compression
  2. Temporal negatives: plan_t must predict plan_{t+1} but NOT plan_{t+2},
     plan_{t+3} from the SAME sequence. This forces positional specificity.
  3. The bottleneck pathway has separate parameters, constraining the gradient
     to a narrow subspace of the transformer's representations.

The protocol's insight: compilers don't optimize raw source code — they lift to
an IR first. NBT forces the model to compress its state into a 64-dim "narrative
IR" at each sentence boundary, then predict the next IR state.

Architecture: same as v10.2 (3L, d=256, d_ff=512) for clean comparison.

Usage:
  python v12/train_v12_nbt.py --minutes 120
"""
...
OUT_DIR = SCRIPT_DIR / 'out_v12_nbt'
...
# NBT-specific
PLAN_STRIDE = 20       # positions 20, 40, 60, 80, 100
D_PLAN = 64            # tight bottleneck (was 128 in SPC)
PLAN_TEMPERATURE = 0.07
PLAN_WEIGHT = 0.1
...
# ============================================================================
# MODEL — v10.2 architecture + NBT bottleneck
# ============================================================================
...
class VortexNBTModel(nn.Module):
    def __init__(self, vocab, d_model, d_ff, n_heads, d_head, n_layers, seq_len,
                 d_plan=D_PLAN, plan_stride=PLAN_STRIDE):
        super().__init__()
        self.seq_len = seq_len
        self.vocab = vocab
        self.plan_stride = plan_stride
        ...
        # NBT bottleneck: separate pathway, tight compression
        self.plan_bottleneck = nn.Linear(d_model, d_plan, bias=False)

        nn.init.normal_(self.embed.weight, std=0.02)

        total = sum(p.numel() for p in self.parameters())
        print(f"  Model: Vortex v12 NBT | {total:,} ({total/1e6:.2f}M)")
        print(f"    d={d_model}, L={n_layers}, H={n_heads}, d_head={d_head}, d_ff={d_ff}")
        print(f"    NBT: stride={plan_stride}, d_plan={d_plan}, temporal negatives")
    ...
    def compute_plan_loss(self, h):
        """
        Narrative Bottleneck Tokens loss.

        At each plan position, compress h through 64-dim bottleneck.
        Plan_t must predict plan_{t+1} (next sentence's narrative state).

        Negatives:
          - Cross-sequence: plan_{t+1} from other batch elements
          - Temporal: plan_{t+2}, plan_{t+3} from SAME sequence
            (forces positional specificity — plan at pos 20 ≠ plan at pos 60)
        """
        B, T, D = h.shape
        stride = self.plan_stride

        # Plan positions: 20, 40, 60, 80, 100
        plan_positions = list(range(stride, T, stride))
        if len(plan_positions) < 2:
            return torch.tensor(0.0, device=h.device)

        # Extract and bottleneck all plan states
        plans = []
        for pos in plan_positions:
            plan = self.plan_bottleneck(h[:, pos, :])  # (B, d_plan)
            plans.append(plan)
        plans = torch.stack(plans, dim=1)  # (B, N_plans, d_plan)

        n_plans = plans.shape[1]

        # Plan-chain InfoNCE: plan_i predicts plan_{i+1}
        total_loss = 0.0
        n_pairs = 0

        for i in range(n_plans - 1):
            z = plans[:, i, :]           # (B, d_plan) — current plan
            pos_target = plans[:, i+1, :]  # (B, d_plan) — next plan (positive)

            # Build negative set:
            # 1. Cross-sequence: plan_{i+1} from other batch elements
            # 2. Temporal: plan_{i+2}, plan_{i+3} from SAME sequence (hard negatives)
            neg_indices = []

            # All other batch elements at position i+1 (cross-sequence negatives)
            for j in range(B):
                row = []
                for k in range(B):
                    if k != j:
                        row.append(pos_target[k])
                # Same-sequence temporal negatives (future plans beyond i+1)
                for t_offset in range(2, min(4, n_plans - i)):
                    row.append(plans[j, i + t_offset])
                neg_indices.append(row)

            # Compute similarities for each batch element
            z_norm = F.normalize(z, dim=-1)
            pos_norm = F.normalize(pos_target.detach(), dim=-1)

            # Positive similarity: (B,)
            pos_sim = (z_norm * pos_norm).sum(dim=-1) / PLAN_TEMPERATURE

            # For each element, compute negative similarities
            losses = []
            for j in range(B):
                neg_feats = torch.stack(neg_indices[j])  # (N_neg, d_plan)
                neg_norm = F.normalize(neg_feats.detach(), dim=-1)
                neg_sim = torch.mv(neg_norm, z_norm[j]) / PLAN_TEMPERATURE  # (N_neg,)

                # InfoNCE: -log(exp(pos) / (exp(pos) + sum(exp(neg))))
                all_sims = torch.cat([pos_sim[j:j+1], neg_sim])
                losses.append(-all_sims[0] + torch.logsumexp(all_sims, dim=0))
...
def train(tokenizer, vocab, train_ds, val_data, minutes):
    ...
    model = VortexNBTModel(vocab=vocab, d_model=D_MODEL, d_ff=D_FF,
    ...
    print(f"  NBT: 64-dim bottleneck, stride={PLAN_STRIDE}, temporal negatives")
    ...
    running_ce = running_plan = running_n = 0
    ...
    while True:
        ...
        plan_loss = model.compute_plan_loss(h)
        total_loss = ce_loss + PLAN_WEIGHT * plan_loss
        ...
        running_plan += plan_loss.item()
        running_n += 1

        if step % LOG_EVERY == 0:
            avg_ce = running_ce / running_n
            avg_plan = running_plan / running_n
            ...
            print(f"  step {step:>5d} | CE {avg_ce:.4f} PPL {math.exp(min(avg_ce,10)):.2f} | "
                  f"PLAN {avg_plan:.3f} | LR {lr:.1e} | tok/s {tokens_seen/elapsed:.0f} | {elapsed/60:.1f}m")
            running_ce = running_plan = running_n = 0
...
def main():
    parser = argparse.ArgumentParser(description="FlashLM Vortex v12 NBT")
    ...
    print("=" * 60)
    print("FlashLM Vortex v12 — Narrative Bottleneck Tokens (NBT)")
