#!/usr/bin/env python3
"""
FlashLM v7.1 CORTEX-III v2 — Optimization Sweep
=================================================
Tests improvements on the winning Local-then-Global architecture.

  Variant A: Local-then-Global + learned position embeddings
  Variant B: Local-then-Global + wider global kernels (k=11 at L3-5)
  Variant C: Local-then-Global + both (position + wider kernels)

  Base: k=[8,8,8,7,7,7], dil=[1,1,1,2,4,8], RF=106 (PPL 44.66 winner)
  7 min per variant = ~21 min total

Usage:  python v7/cortex3_v3_experiment.py --minutes 7
"""
...
class GatedConvModel(nn.Module):
    ...
    def __init__(self, vocab, d_model, n_layers, d_ff, seq_len,
        ...
        pos_str = " + pos_emb" if use_pos_emb else ""
        print(f"    RF: {rf} tok | k={kernels} | d={dilations}{pos_str} | Params: {total_params:,}")
...
# ============================================================================
def main():
    parser = argparse.ArgumentParser(description="FlashLM v7.1 CORTEX-III v2 Optimization")
    parser.add_argument('--minutes', type=float, default=7, help='Minutes per variant')
    args = parser.parse_args()

    print(f"\n{'=' * 60}")
    print(f"  CORTEX-III v2 — Optimization Sweep")
    print(f"{'=' * 60}")
    ...
    # Base schedule: Local-then-Global (winner from v2 experiment)
    base_k = [8, 8, 8, 7, 7, 7]
    base_d = [1, 1, 1, 2, 4, 8]

    # ========================================================================
    # Variant A: + Position Embeddings
    # ========================================================================
    print(f"\n{'#' * 60}")
    print(f"  Variant A: Local-then-Global + Position Embeddings")
    print(f"{'#' * 60}")
    model_a = GatedConvModel(vocab, D_MODEL, N_LAYERS, D_FF, SEQ_LEN,
                              kernels=base_k, dilations=base_d, use_pos_emb=True)
    r_a = run_experiment("L-then-G + PosEmb (RF=106)", model_a, train_ds, val_data, tokenizer, args.minutes)
    results.append(r_a)
    del model_a; gc.collect()

    # ========================================================================
    # Variant B: + Wider Global Kernels (k=11 at L3-5)
    #   RF = 1 + 7+7+7 + 20+40+80 = 169 tokens
    # ========================================================================
    print(f"\n{'#' * 60}")
    print(f"  Variant B: Local-then-Global + Wider Global (k=11)")
    print(f"{'#' * 60}")
    model_b = GatedConvModel(vocab, D_MODEL, N_LAYERS, D_FF, SEQ_LEN,
                              kernels=[8, 8, 8, 11, 11, 11],
                              dilations=base_d, use_pos_emb=False)
    r_b = run_experiment("L-then-G Wide (k=11, RF=169)", model_b, train_ds, val_data, tokenizer, args.minutes)
    results.append(r_b)
    del model_b; gc.collect()

    # ========================================================================
    # Variant C: + Both (position embeddings + wider kernels)
    # ========================================================================
    print(f"\n{'#' * 60}")
    print(f"  Variant C: Local-then-Global + PosEmb + Wider Global")
    print(f"{'#' * 60}")
    model_c = GatedConvModel(vocab, D_MODEL, N_LAYERS, D_FF, SEQ_LEN,
                              kernels=[8, 8, 8, 11, 11, 11],
                              dilations=base_d, use_pos_emb=True)
    r_c = run_experiment("L-then-G + PosEmb + Wide (RF=169)", model_c, train_ds, val_data, tokenizer, args.minutes)
    results.append(r_c)
    ...
    print(f"{'=' * 85}")
    print(f"  {'Variant':<42} {'Params':>8} {'Best PPL':>10} {'Final PPL':>10} {'Tok/s':>8}")
    print(f"  {'-' * 78}")
    for r in results:
        tps = r['tokens'] / max(r['time_min'] * 60, 1)
        print(f"  {r['name']:<42} {r['params']:>8,} {r['best_ppl']:>10.2f} {r['final_ppl']:>10.2f} {tps:>8,.0f}")
    print(f"{'=' * 85}")
    print(f"  Reference: Local-then-Global baseline = PPL 44.66, 3,461 tok/s")
    ...
    json.dump(results, open(out_dir / 'cortex3_v3_results.json', 'w'), indent=2)
    print(f"\n  Saved to {out_dir}/cortex3_v3_results.json\n")
